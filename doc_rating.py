...
for i, test_sentence in enumerate(test_sentences):
    ...
    score = project(resp["embedding"], model)
    errors[i] = np.abs(score - test_sentence['label'])
    print(f"Alignment score: {score}\nLabel score: {test_sentence['label']}")

print(f"with fitted line on the embeddings from {EMBEDDING_MODEL} the score deviates in average {np.mean(errors)} from the labels")
...
for i, test_sentence in enumerate(test_sentences):
    resp = ollama.embeddings(model=EMBEDDING_MODEL, prompt=test_sentence['text'])
    score = project_reduced(np.asarray(resp["embedding"]), pca, model_reduced)
    errors[i] = np.abs(score - test_sentence['label'])
    print(f"Alignment score: {score}\nLabel score: {test_sentence['label']}")
...
#     return { "text": "r2": r2, "mae": mae, "spearman": spearman, "max_err": max_err}


# # Evaluate on both sets
# train_metrics = evaluate_probe(y_true_train, y_pred_train, "Anchor (Train)")
# test_metrics  = evaluate_probe(y_true_test,  y_pred_test,  "Test")
